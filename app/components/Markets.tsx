  ...
  return (
    <div className="flex flex-col flex-1 max-w-[1280px] w-full">
      <div className="flex flex-col flex-1 w-full overflow-x-auto">
        <div className="flex flex-col w-full min-w-0 rounded-xl overflow-hidden border border-white/10 bg-[#0a0a0a]">
    ...
    >
      <td className="px-3 sm:px-5 py-3 sm:py-3.5">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm font-semibold text-white">{market.symbol}</p>
            <p className="text-[10px] text-white/30">{market.symbol}</p>
          </div>
        </div>
      </td>
      <td className="px-3 sm:px-5 py-3 sm:py-3.5">
        <span className="font-mono text-xs sm:text-sm text-white tabular-nums">{market.lastPrice}</span>
      </td>
      ...
      <td className="px-3 sm:px-5 py-3 sm:py-3.5">
        <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded text-[10px] sm:text-xs font-mono font-semibold ${
          isPositive
    ...
    <thead>
      <tr className="border-b border-white/10">
        {[
          { label: "Name",       hide: "" },
          { label: "Price",      hide: "" },
          { label: "24h High",   hide: "hidden sm:table-cell" },
          { label: "24h Volume", hide: "hidden sm:table-cell" },
          { label: "24h Change", hide: "" },
        ].map(({ label, hide }) => (
          <th key={label} className={`px-3 sm:px-5 py-3 text-left text-[9px] font-semibold uppercase tracking-[0.15em] text-white/30 ${hide}`}>
            {label}
          </th>
        ))}
      </tr>
